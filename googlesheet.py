...
def update_sheet(url, i):
    ...
    work_sheet = google_sheet.open("M 0{}".format(i + 1) if i < 9 else "M {}".format(i + 1)).worksheet("2020/05")
    ...
    if url[8:20] == "item.mercari":
        ...
        work_sheet.update_acell("I{}".format(new_link_index + 4), format(str(datetime.date.today())))
